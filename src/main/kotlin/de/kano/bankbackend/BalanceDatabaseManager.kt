...
fun transferBalance(withdrawAccountNumber: Long, depositAccountNumber: Long, transfer: Double): Any {
    //val dbConnection = DriverManager.getConnection("jdbc:sqlite:database.db")
    //dbConnection.use {
    ...
    //    statement2.execute()
    val newBalanceDepositted = depositBalance(depositAccountNumber, transfer)
    val newBalanceWithdrawn = withdrawBalance(withdrawAccountNumber, transfer)

    data class NewBalance(val newBalanceDepositted: Double, val newBalanceWithdrawn: Double)

    return NewBalance(newBalanceDepositted, newBalanceWithdrawn)
